...
# ─────────────────────────────────────────────────────────────────────────────
# Setup script — bootstraps the development environment
...
install_deps() {
    ...
    if [[ "$PROD_MODE" != "--prod" ]]; then
        ...
        "$VENV_DIR/bin/pre-commit" install --quiet || warn "pre-commit install skipped"
    fi
}
...
main() {
    log "Bootstrapping project environment..."
    check_python
    create_venv
    install_deps
    verify_install
